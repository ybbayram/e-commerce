						...
						<div class="card-body">
							...
							<div class="row">
								...
								<div class="col-lg-12 col-sm-12 mb-4">
									<form action="{{route('admin.promosyon.guncelle', $promosyon->id)}}" method="post">
										@csrf
										<div class="form-group ">
											<label for="example-email-input1" class="col-form-label pt-0">Ad</label>
											<input class="form-control" name="ad" type="text" id="example-email-input1" placeholder="Kampanya adının giriniz." required="true" value="{{ old('min', $promosyon->ad) }}">
										</div> 
										<div class="form-group ">
											<label for="example-email-input1" class="col-form-label pt-0">Sepet Minimum Tutar</label>
											<input class="form-control" name="min" type="number" step="any" id="example-email-input1" placeholder="Sepet Minimum Tutar" required="true" value="{{ old('min', $promosyon->min) }}">
										</div>

										<div class="form-group">
											<label class="mt-2">İndirim Türü</label>
											<select class="form-control mb-3" id="uyg_tur" onchange="uygulamaTuru()" name="uyg_tur" disabled style="width: 100%">
												<option>Tür Seçiniz</option> 
												<option value="4" @if($promosyon->grup == 4) selected @endif>Tüm Ürünler</option> 
												<option value="5" @if($promosyon->grup == 5) selected @endif>Ürünler</option> 
												<option value="0" @if($promosyon->grup == 0) selected @endif>Kategori</option> 
												<option value="1" @if($promosyon->grup == 1) selected @endif>Alt Kategori</option> 
												<option value="2" @if($promosyon->grup == 2) selected @endif>Marka</option> 
												<option value="3" @if($promosyon->grup == 3) selected @endif>Etiket</option> 
											</select> 
										</div> 
										...
										<div class="form-group"  id="kategori" style="display: none;">
											...
											<select class="select2 mb-3 select2-multiple" name="kategoriler[]" style="width: 100%" multiple="multiple" >
												...
												<option value="{{ $entry->id }}"
													@foreach($promosyonDetay as $detay)
													...
													<div class="form-group " id="urunler" style="display: none;" >
														...
														<div class="form-group " >
															...
															<select class="select2 mb-3 select2-multiple" name="pro_urunler[]" style="width: 100%" multiple="multiple" >
																@foreach($urunler as $entry) 
																<option value="{{ $entry['id'] }}"
																@foreach($promosyonUrun as $urun)
																@if($urun->urun_id == $entry->id) selected @endif
																@endforeach
																>{{ $entry['baslik'] }}</option>
																@endforeach
															</select> 
