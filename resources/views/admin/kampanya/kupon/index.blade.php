		...
		<div class="container-fluid">
			...
			<div class="row">
				...
				<div class="col-xl-12">
					<div class="card">
						<div class="card-body"> 
							<a href="{{route('admin.kupon.olustur.sayfa')}}"><span class="btn btn-primary btn btn-xl inputTwo" style="margin-bottom: 20px">Yeni Kupon</span></span></a>
							<span class="float-right text-muted font-size-13"></span>
							<h5 class="card-title mb-3">Kuponlar</h5>
							<p class="card-title-desc">
								Kuponlara bu sayfadan ulaşabilirsiniz.
							</p>
							...
							<div class="card-body">
								<div class="table-responsive">
									<table id="datatable-buttons" class="table table-striped table-bordered w-100">
										...
										<tbody>
											@foreach($kuponlar as $entry)
											<tr>
												<td>{{$entry->ad}}</td>
												<td>{{$entry->adet}} / {{$entry->kupon_kullanimi->count()}}</td>
												<td>{{$entry->min}}</td>
												<td>{{$entry->indirim_tutarı}}</td>
												<td>{{$entry->baslangic_tarihi}}</td>
												<td>{{$entry->bitis_tarihi}}</td>
												<td>{{ $entry['created_at'] }}</td>
												<td>
													<a href="{{route('admin.kupon.goster.sayfa', $entry->id)}}" title="Kuponlar" data-tippy-placement="top" class="btn btn-primary waves-effect waves-light tippy-btn"><i class="dripicons-ticket"></i></a>  
													@if($entry->durum == 0)
													<a href="{{ route('admin.kupon.aktifYap', $entry->id) }}" class="btn btn-danger waves-effect waves-light">Aktif Yap</a>
													@else
													<a href="{{ route('admin.kupon.pasifYap', $entry->id) }}" class="btn btn-success waves-effect waves-light">Pasif Yap</a>
													@endif
														...
														<div class="modal-dialog modal-dialog-centered">
															<div class="modal-content">
																...
																<div class="modal-body">
																	<form action="{{ route('admin.kupon.sil', $entry->id) }}" method="get">
																		@csrf
