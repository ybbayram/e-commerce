		...
		<div class="container-fluid">
			...
			<div class="row">
				...
				<div class="col-xl-12">
					<div class="card">
						<div class="card-body"> 
							<a href="{{route('admin.kargo.olustur.sayfa')}}"><span class="btn btn-primary btn btn-xl inputTwo" style="margin-bottom: 20px">Yeni Kargo Kampanyası</span></span></a>
							<span class="float-right text-muted font-size-13"></span>
							<h5 class="card-title mb-3">Kargo İndirimi</h5>
							<p class="card-title-desc">
								Kargo indirimi kampanyalarına bu sayfadan ulaşabilirsiniz.
							</p>
									...
									<table id="datatable-buttons" class="table table-striped table-bordered w-100">
										...
										<tbody>
											...
											<tr>
												<td>{{$entry->ad}}</td>
												<td>{{$entry->min}}</td>
												<td>% {{$entry->indirim_oranı}}</td>
												<td>{{$entry->baslangic_tarihi}}</td>
												<td>{{$entry->bitis_tarihi}}</td>
 												<td>{{ $entry['created_at'] }}</td>
 												<td>
													<a href="{{ route('admin.kargo.guncelle.sayfa',$entry->id) }}" title="Düzenle" data-tippy-placement="top" class="btn btn-primary waves-effect waves-light tippy-btn"><i class="dripicons-pencil"></i></a>
														...
														<div class="modal-dialog modal-dialog-centered">
															<div class="modal-content">
																...
																<div class="modal-body">
																	<form action="{{ route('admin.kargo.sil', $entry->id) }}" method="get">
																		@csrf
