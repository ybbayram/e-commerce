		...
		<div class="container-fluid">
			...
			<div class="row">
				...
				<div class="col-xl-12">
					<div class="card">
						<div class="card-body"> 
							<a href="{{route('admin.sepetIndirim.olustur.sayfa')}}"><span class="btn btn-primary btn btn-xl inputTwo" style="margin-bottom: 20px">Yeni Sepet Kampanyası</span></span></a>
							<span class="float-right text-muted font-size-13"></span>
							<h5 class="card-title mb-3">Sepet İndirimi</h5>
							<p class="card-title-desc">
								Sepet indirimi kampanyalarına bu sayfadan ulaşabilirsiniz.
							</p>
							...
							<div class="card-body">
								...
								<div class="table-responsive">
									<table id="datatable-buttons" class="table table-striped table-bordered w-100">
										...
										<tbody>
											@foreach($sepetIndirim as $entry)
											<tr>
												<td>{{$entry->ad}}</td>
												<td>{{$entry->min}}</td>
												<td>@if($entry->tur == 0) Yüzdelik İndirim @endif @if($entry->tur == 1) Tutar İndirimi @endif</td>
												<td>@if($entry->tur == 0)%@endif {{$entry->indirim_oranı}}</td>
												<td>{{$entry->baslangic_tarihi}}</td>
												<td>{{$entry->bitis_tarihi}}</td>
												<td>{{$entry['created_at']}}</td>
												<td>
													<a href="{{ route('admin.sepetIndirim.guncelle.sayfa',$entry->id) }}" title="Düzenle" data-tippy-placement="top" class="btn btn-primary waves-effect waves-light tippy-btn"><i class="dripicons-pencil"></i></a>
														...
														<div class="modal-dialog modal-dialog-centered">
															<div class="modal-content">
																...
																<div class="modal-body">
																	<form action="{{ route('admin.sepetIndirim.sil', $entry->id) }}" method="get">
																		@csrf
