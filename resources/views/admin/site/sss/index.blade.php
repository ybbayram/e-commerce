		...
		<div class="container-fluid">
			...
			<div class="row">
				<div class="col-xl-12">
					<div class="card">
						<div class="card-body">
							<a href="{{ route('admin.sss.olustur.sayfa') }}"><span class="btn btn-primary btn btn-xl inputTwo" style="margin-bottom: 20px">Yeni SSS</span></span></a>
							<a href="{{ route('admin.sss.sirala.sayfa') }}"><span class="btn btn-primary btn btn-xl inputTwo" style="margin-bottom: 20px">Sırala</span></span></a>
							<span class="float-right text-muted font-size-13"></span>

							<h5 class="card-title mb-3">SSS</h5>
							...
							<div class="card-body">
								<div class="table-responsive">
									<table id="datatable-buttons" class="table table-striped table-bordered w-100">
										...
										<tbody>
											@foreach($sss as $entry)
											<tr>
												<td>{{ $entry['baslik'] }}</td>
												<td>{{ $entry['created_at'] }}</td>
												<td>
													<a href="{{route('admin.sss.detay', $entry->id)}}" title="Soru Ekle" data-tippy-placement="top" class="btn btn-primary waves-effect waves-light tippy-btn"><i class="dripicons-network-3"></i></a>
													<a href="{{ route('admin.sss.guncelle.sayfa', $entry->id) }}" title="Düzenle" data-tippy-placement="top" class="btn btn-primary waves-effect waves-light tippy-btn"><i class="dripicons-pencil"></i></a>
													<a href="{{ route('admin.sssDil', $entry->id) }}" title="Dil"  class="btn btn-primary waves-effect waves-light tippy-btn"><i class="dripicons-flag"></i></a>
													@if($entry->durum == 0)
													<a href="{{ route('admin.sss.aktifYap', $entry->id) }}" class="btn btn-danger waves-effect waves-light">Aktif Yap</a>
													@else
													<a href="{{ route('admin.sss.pasifYap', $entry->id) }}" class="btn btn-success waves-effect waves-light">Pasif Yap</a>
													@endif
														...
														<div class="modal-dialog modal-dialog-centered">
															<div class="modal-content">
																...
																<div class="modal-body">
																	<form action="{{ route('admin.sss.sil', $entry->id) }}" method="get">
																		@csrf
